...
module.exports = {
  list: async () => {
    ...
    if (!resp.data) throw new ClientError("Planets no encontrados", 400);
    return resp.data;
  },
  getByID: async (id) => {
    const resp = await axios.get(`${URL_CONNECTION}/${id}`);
    if (!resp.data) throw new ClientError("Planet no encontrado", 400);
    return resp.data;
  },
  create: async (data) => {
    const resp = await axios.post(`${URL_CONNECTION}`, data);
    if (!resp.data) throw new ClientError("Planet existente", 400);
    return resp.data;
  },
  eliminate: async (id) => {
